...
**Commit provenance (do not conflate):** `6a6cddd` is the **pre-change baseline** (the website's current pin; contains none of D2/D3/D4). `2daa3ff` is the **local review-fonts implementation commit** in this workspace (55 files). The **final source tree** for this release is `2daa3ff` + the release-pass changes of 12 September (reviewer-CLI strictness and exit codes, `--verify-note`, placeholder-configuration proof, Yarn-only lockfile, tracked `frontend/app/review-access.tsx`, release backend-origin resolution, handoff docs); its content digest is recorded in `HANDOFF_MANIFEST.json` (`source_tree.digest_sha256`). The **final source commit SHA does not exist yet** — it is created by the owner's *Save to GitHub* after this package; record THAT SHA (never `6a6cddd`, and not the digest) as the website's new app pin and as the app's `BUILD_COMMIT`. Production still runs the older build until the owner redeploys; deployment and login status are listed in the manifest as *pending / unverified*, not as done.
