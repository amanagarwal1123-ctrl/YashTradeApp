...
> **14 Sep 2026 — privacy wording + outbox contract detail.** `WEBSITE_PRIVACY_UPDATE.md` (in this handoff) contains the exact replacement text for the website privacy policy: AI recipients and what is transferred, consent withdrawal/history deletion, staff uploads, deletion/retention (resolves "completed within 30 days" vs indefinite pending), staff accounts, store-review accounts. Reviewer storage = `review__*` collections in the app's main database — **no separate review database, no `REVIEW_DB_NAME`, no website reviewer-auth API**. Contract change: `account_erased` events now carry `required_acknowledgements:["website"]`, so your acknowledgement completes the **app + website cleanup** outcome (`cleanup_completed`); provider erasure is a separate, manually requested ledger outcome the ack never touches (privacy wording §4). Nothing else changed for the website.

## Current build — 13 September 2026 (`shared-v1-store-submission-2026-09-13`)

**What changed for the website since the 12 September build: nothing it has to implement.** The 13 Sep store-submission work is app/backend-internal: store-review isolation moved from a separate review database to `review__*` prefixed collections inside the same `DB_NAME` (`REVIEW_ACCESS_ENABLED=true`; the `REVIEW_DB_NAME` setting no longer exists), an owner-only console for reviewer keys (`/api/admin/review/*`, production-scope owner admin session + fresh OTP), AI consent (`/api/ai/consent`, `403 AI_CONSENT_REQUIRED` on `/api/ai/chat`) and deeper account deletion. Facts the website should know: (1) reviewer identities live only in `review__users`, are never returned by `/integrations/customers/*` lookups and never appear in the deletion outbox for the website's consumer; (2) a customer's account deletion now also removes analytics, AI history and consent on the app side — the D4 outbox event shape is unchanged; (3) D2/D3/D4, staff exchange, `flows.owner_admin` and every health/readiness shape below are unchanged and remain the contract. `contracts/openapi.shared-v1.json` regenerated from this tree lists the new `/api/admin/review/*` and `/api/ai/consent` operations; the website does not call them.

## Previous build — 12 September 2026 (`shared-v1-review-fonts-2026-09-12`)

This build resolves the three website dependencies the website kept as strict XFAIL against app pin `6a6cddd` (**D2** customer-search route, **D3** exact `customer_id` history filter, **D4** consumer-scoped deletion outbox) — contract below — and adds isolated store-review access (`STORE_REVIEW_ACCESS.md`), a fixed icon-font fallback asset, and the owner-operated production runbook (`PRODUCTION_ADMIN_RECOVERY.md`: the owner edits production settings in Manage Publishes → Secrets and has database access; no other operator is needed). The regenerated `contracts/openapi.shared-v1.json` describes these routes.

**Commit provenance (do not conflate):** `6a6cddd` is the **pre-change baseline** (the website's current pin; contains none of D2/D3/D4). The release content is the **source commit** recorded in `HANDOFF_MANIFEST.json` → `source_provenance.source_commit.sha`: review-fonts implementation (`2daa3ff`), release pass (`1370f0a`: reviewer-CLI strictness and exit codes, `--verify-note`, placeholder-configuration proof, Yarn-only lockfile, tracked `frontend/app/review-access.tsx`, release backend-origin resolution) and the 12 September production hotfix (optional review storage can no longer crash startup; `--verify-note` pins the backend URL; `frontend/yarn.lock` tracked). Every hash in the manifest is taken from that commit's **Git objects** — per file `git_blob_sha1` (the blob id GitHub shows) and the SHA-256 of the blob bytes — plus a digest of the whole committed tree; the commit that follows adds only `WEBSITE_RELEASE_HANDOFF.zip`, and a platform "Auto-generated changes" commit may touch `.emergent/` only. Once the owner's *Save to GitHub* has pushed it, confirm the source commit exists on GitHub with the same SHA and blob ids, then record THAT SHA (never `6a6cddd`, and not the digest) as the website's new app pin and as the app's `BUILD_COMMIT`. Production still runs the older build until the owner redeploys; deployment and login status are listed in the manifest as *pending / unverified*, not as done.

**Website configuration is the website agent's `.env` job.** Before anything else, the website's agent must declare in the website's own backend `.env`: `CANONICAL_API_BASE_URL`, `ENROLLMENT_INTEGRATION_KEY`, `STAFF_SERVICE_KEY` (non-working placeholders for the two secrets) and `BFF_ALLOWED_ORIGINS` listing **both** `https://register.yashsilver.com` and `https://yash-register.emergent.host`; then republish (names appear in that project's Secrets) → owner enters values → republish again. Existing production keys take their value from Secrets, not from `.env`. Readiness `false` = missing/invalid setting, not "backend unreachable". Full text: `WEBSITE_AUTH_FIX_PROMPT.md`.

Owner-login correction: see **PRODUCTION_ADMIN_RECOVERY.md** and copy/paste **WEBSITE_AUTH_FIX_PROMPT.md**. Production read-only observation remains `shared-v1-followup-2026-09-11`, staff key absent; both website domains lack canonical base/enrollment/staff settings. No production role/settings have been changed here yet. An authenticated production GET confirmed 9999813334 is active/verified/customer, ID `bcdf18c9-dc87-4d46-b580-30cf519103df`; **since 12 September the canonical backend promotes that same record to admin automatically on its first start after this build is deployed** (`flows.owner_admin` in `/api/health`), so the website only has to complete its own settings and route canonical `role=admin` to its admin experience. It does not require the full website identity export; other identity merge/export gates below are unchanged.

No-SMS checks: `/api/health/live` (process only), `/api/health/ready` and `/api/health` (503 when any non-optional auth flow is unready; `flows.review` is optional), `/api/integrations/staff/readiness` + `X-Staff-Service-Key`, `/api/integrations/enrollment/readiness` + `X-Integration-Key`. Protected checks establish credential matching, never delivery/role. Mobile is independent of missing staff settings. Keep both website origins allowed in the BFF, not just app CORS.

### Website dependencies D2 / D3 / D4 — resolved contract

Website tests that should flip from XFAIL to PASS once the website pins the new commit: `tests/shared_v1/test_bff_followup_core_regressions.py` — the D2 customer-search, D3 customer-ID history and D4 outbox-starvation/acknowledgement cases. The app-side equivalents are `backend/tests/shared/test_website_dependencies_d2_d3_d4.py` (9 tests, passing).

| | Route | Contract |
|---|---|---|
| **D2** | `GET /api/customers/search?q=<term>` — bearer **admin or billing_executive** (telecaller/customer 403, anonymous 401) | Registered before `GET /customers/{uid}`, so the literal `search` segment is never treated as a customer ID. Case-insensitive match on phone, name, customer_code/code, city, location, shop_name; `role=customer` only; deleted customers excluded (inactive included). Response `{customers:[public customer rows], query, limit:20, minimum_length:2}`; terms under 2 characters return `customers:[]` (no directory enumeration). Rows never include `session_version`/`identity_events`. `GET /customers/{uid}` (admin) is unchanged and now returns `detail_limit:100` plus `complete_history:"/api/requests?customer_id=<id>&status=all&sort=newest"`. |
| **D3** | `GET /api/requests?customer_id=<canonical id>&page=&limit=&status=&request_type=&sort=` — bearer staff | Exact, immutable canonical-ID scope applied after enrichment, so legacy `user_id`-backed rows and `customer_id` rows both qualify; never matches by name/phone snapshot. Composes with all existing filters; envelope unchanged (`requests,total,page,limit,pages,open_counts_by_type,server_time`) plus `customer_id` echo. Malformed value → 422 `INVALID_FILTER` (`^[A-Za-z0-9_.:-]{1,64}$`); unknown ID → 404 `CUSTOMER_NOT_FOUND`; deleted customers keep their anonymised history. Paginate with `limit<=100`; no 100-record ceiling on total. |
| **D4** | `GET /api/integrations/deletions?limit=<1..500>&after=<cursor>` and `POST /api/integrations/deletions/{event_id}/ack` — server-only `X-Integration-Key` | Lists only `account_erased` events **not yet acknowledged by the calling consumer** (`consumer` is derived from the credential — a `?consumer=` query is ignored), stable `(created_at,id)` order, opaque `next_cursor`, `has_more`, `remaining_for_consumer`; the 101st event is visible after 100 acknowledgements. Invalid cursor / `limit` out of range → 422. `ack` is idempotent and consumer-specific: `{acknowledged, event_id, acknowledged_by[], required_acknowledgements[], all_acknowledged}`; global `status` becomes `acknowledged` only when every required consumer has acknowledged — **14 Sep 2026: `required_acknowledgements` is `["website"]` only**, so the website's ack completes the app + website CLEANUP outcome (`all_acknowledged:true`, app-side `deletion_requests.status=cleanup_completed`) — provider-held copies are a separate ledger outcome and are never marked erased by the ack; unknown event 404. Acknowledged tombstones are never resurrected by a later enrollment attempt. |

## Previous phase delivery record

Workspace build: **`shared-v1-followup-2026-09-11`**, preview `https://yash-review-deploy.preview.emergentagent.com` (recorded commit: unrecorded). Public main baseline **30796997d3484594c6c5f53965e1c71dd5ed1c86** was verified before editing; prior claims that this baseline sync was unverified are superseded. Follow-up changes require the user-controlled Save to GitHub action; no new implementation commit/remote verification is available in this environment. Do not call the baseline the new implementation commit. No production deployment performed. Last independent production app observation: `https://yash-tryon-test.emergent.host/api/health`, **2026.09.09-integration-v7**, commit unknown; website health observations **2026.09.10-login-v10**, commits unknown. Code sync is not rollout.
...
4. Step 2 confirms enrollment and shows actual Android/iOS download links with icons, explaining login in Yash Trade with the same phone and a **fresh OTP**. **15 Sep 2026:** customers may also sign up directly in the app (phone + OTP creates the account, profile completed in-app). A later website enrollment for that number **updates the same account** (`created:false`) — website values overwrite and the app lets the customer choose per field where they differ; the website should prefill from `GET /integrations/customers/{phone}` when it exists. A number whose account was deleted can register again with a fresh OTP (new identity); only grants issued before the deletion are refused (`409 DELETED_IDENTITY`). No fictional username/password. Customers do not receive an administration portal.
...
Owner phone **9999813334** is the **default administrator on both surfaces** (owner decision, 12 September 2026): the canonical backend applies it itself at every start — `OWNER_ADMIN_PHONE=9999813334`, `backend/shared/owner_admin.py` — promoting the existing record to admin on its existing canonical ID with history preserved (or creating it where absent), never merging identities. `/api/health` reports `flows.owner_admin.state` (`created` / `promoted` / `already_admin`). The website reads that role from canonical `/auth/me` after the staff OTP exchange; it must **not** create a website-local admin or a phone/password login for it, and must not independently promote website records. Other roles and any broader identity reconciliation still require explicit approved mappings, backup and owner approval of the report hash.
...
The website must also have its own private session/cookie signing configuration. No NEXT_PUBLIC/EXPO_PUBLIC/client bundle should contain either integration key, signing key, storage key or refresh token. Secret-value presence/rotation receipts are private; publish only names/status/outcomes. Production values are set by the **owner** in Emergent *Manage Publishes → Secrets* after the agent has declared the names in the project's preview `.env` (app: `STAFF_SERVICE_KEY`, `REVIEW_DB_NAME`, `BUILD_COMMIT` are declared; `frontend/.env` declares `EXPO_PUBLIC_PRODUCTION_BACKEND_URL`), in the sequence declare → republish → owner edits values → republish. Placeholders are treated as absent by the code (fail closed). The exact owner procedure is `PRODUCTION_ADMIN_RECOVERY.md`.
...
Do not modify protected Expo proxy/hostname variables. App uses EXPO_PUBLIC_BACKEND_URL and EXPO_PUBLIC_ENROLLMENT_URL (REGISTRATION_URL accepted by config adapter). Release builds (`NODE_ENV=production`) resolve the backend origin to `https://yash-tryon-test.emergent.host` (`EXPO_PUBLIC_PRODUCTION_BACKEND_URL`) and never ship a preview-container origin; the deployment's `EXPO_PUBLIC_BACKEND_URL` Secret should also be set to that value. Non-secret configuration names, limits and capabilities are listed in the shared contract.
...
- Store-review identities: `review__*` prefixed collections inside the same database (application-enforced by session scope; no separate review database), four reviewer roles, owner console in the app + owner-run CLI with private note and deployed-backend verification (`STORE_REVIEW_ACCESS.md`). Provisioned and verified in **preview** only (accounts revoked afterwards); production provisioning is an owner action. No universal OTP or public reviewer account list exists; reviewer identities are invisible to the website's integration lookups.
- Local branch **main**; verified pre-follow-up baseline **30796997d3484594c6c5f53965e1c71dd5ed1c86**; the website's current pin `6a6cddd` predates D2/D3/D4. No Git remote/write credential is configured in this container; the user-controlled Save to GitHub operation is required to obtain/verify the NEW implementation commit and pinned links. Then stage both clients against that exact contract, record BUILD_COMMIT, confirm genuine owner continuity/backup/secrets/reviewer/device/provider gates, agree maintenance window, apply only separately approved reconciliation, revoke old client sessions and require fresh OTP. Never deploy breaking v1 OTP/grant/session behavior to the unprepared live website merely to fill a report field.