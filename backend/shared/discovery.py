...
async def page_of(session, page, limit):
    ...
    return {"session_id": session["id"], "products": items, "page": page, "limit": limit, "total": total,
            ...
            "exhausted": session["unseen_count"] == 0, "expires_at": session["expires_at"].isoformat() if hasattr(session["expires_at"], "isoformat") else session["expires_at"],
...
async def start(req: Start, user=Depends(c.current_user)):
    ...
    ids = [p["id"] async for p in c.db.products.find(query, {"_id": 0, "id": 1}).sort([("created_at", -1), ("id", 1)]).limit(MAX_SESSION_IDS)]
    ...
    unseen = [pid for pid in ids if pid not in seen_at]
    seen = sorted((pid for pid in ids if pid in seen_at), key=lambda pid: (seen_at[pid], pid))  # least recently seen first
    ...
    doc = {"id": secrets.token_hex(12), "user_id": user["id"], "filters": {"metal_type": req.metal_type, "category": req.category, "post_type": req.post_type},
           ...
           "truncated": len(ids) == MAX_SESSION_IDS}
