...
def _body(new_phone, expected="9000000001", uid=TELE, key=None, reason="Handset lost; new company SIM issued to the telecaller"):
...
async def test_unused_number_change_preserves_identity_revokes_sessions_and_requires_verification(api_client, isolated_db, login_helper):
    ...
    key = uuid.uuid4().hex
    res = await api_client.post(f"/api/integrations/staff/{TELE}/phone", headers=_auth(admin), json=_body("+61 412 345 678", key=key))
    ...
    again = await api_client.post(f"/api/integrations/staff/{TELE}/phone", headers=_auth(admin), json=_body("+61 412 345 678", key=key))
    ...
    mismatch = await api_client.post(f"/api/integrations/staff/{TELE}/phone", headers=_auth(admin), json=_body("9111111111", expected="+61412345678", key=key))
...
async def test_recent_authentication_is_required(api_client, isolated_db, login_helper):
    ...
    assert (await api_client.post(f"/api/integrations/staff/{TELE}/phone", headers=_auth(fresh), json=_body("9111111111"))).status_code == 200


@pytest.mark.asyncio
async def test_concurrent_submissions_produce_exactly_one_change(api_client, isolated_db, login_helper):
    admin = await _admin(login_helper)
    bodies = [_body("9111111111"), _body("9222222222"), _body("9333333333")]
