...
agent_communication:
    ...
    - agent: "main"
    - message: "21 Sep 2026 14:00 UTC FIRST-OPEN NOTIFICATION PROMPT + BELL (owner request). New src/components/FirstOpenNotificationPrompt.tsx rendered from app/_layout.tsx: on the very first open (before sign-in, Android + iOS) a bottom sheet (testIDs first-open-notification-prompt, first-open-notification-continue, first-open-notification-later) -> Continue calls push.requestPermission() (OS dialog); Not now / backdrop / back marks AsyncStorage 'notif_prompt_seen'; never shown again at launch, never on web/simulator (pushSupported() false) or when the OS already decided. src/push.ts adds requestPermission(), firstOpenPromptDue(), markFirstOpenPromptSeen(). Bell: src/components/NotificationBell.tsx (unread badge testID <id>-badge from GET /notifications/inbox?page=1&limit=1) on customer Home notifications-btn, telecaller leads header tc-alerts, Panel header panel-alerts; workspace requests-alerts button shows 'Alerts (n)'. app/notifications.tsx: recent 100 (limit=100, pager removed), tappable 'Turn on notifications' banner testID notifications-permission-card (label testID notifications-allow or notifications-open-settings) -> OS dialog or Linking.openSettings(); banner hidden on web. Jest 27 suites / 162 passed; tsc clean. Web smoke on review scope: badge 1 -> row -> read -> badge cleared (test_reports/bell_home_2026-09-21.jpeg, bell_notifications_2026-09-21.jpeg). The OS dialog / sheet is NOT testable in the web preview - device build only. Synthetic review-scope campaign 13e771e439bba43db5d07f4d ('Bell smoke 21 Sep', 6 users / 0 devices) was sent for the smoke; production untouched."
    - agent: "main"
    - message: "24 Sep 2026 APP STORE REJECTION FIX (owner choices: iOS-only guest preview capped at 100 products, only the catalogue public, Android login-first unchanged). Apple rejected iOS 1.0.2 (107): 5.1.1(v) registration before browsing products; 2.3.10 Google Play references. CODE: src/navigation.ts guestHomeRoute() = /guest-preview on iOS else /login (homeRouteFor default); NEW app/guest-preview.tsx (public GET /products pages of 20, cap 100, end card with total + Sign in; testIDs guest-sign-in, guest-preview-note, guest-item-<id>, guest-list, guest-preview-end, guest-end-sign-in, guest-error, guest-retry, guest-help-link); NEW src/hooks/useRequireSignIn.ts (guest tap on cart/wishlist/Ask Price/Video Call/Hold/Reorder -> Alert 'Sign in to continue' -> /login; window.confirm on web) wired in app/product/[id].tsx (+ /wishlist GET skipped for guests) and app/image-viewer.tsx; app/login.tsx iOS-only login-browse-btn 'Browse the collection' + footer 'Yash Trade App - Wholesale silver & gold jewellery' (EN/HI/PA); profile logout + delete-account -> guestHomeRoute(). WORDING: help.tsx (3 langs), review-access.tsx, review-keys.tsx no longer say Google Play (0 hits in app/ + src/). TESTS: Jest 29 suites / 170 passed (new guestPreview.test.tsx 4, guestGate.test.tsx 4; navigationSession.test.tsx platform split), tsc clean, eslint 0 errors; testing_agent iteration 40 (frontend web) 8/8 PASS. Web cannot run as iOS: the iOS routing itself needs a TestFlight check by the owner. No Git action by the agent."
