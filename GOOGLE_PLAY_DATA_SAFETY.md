...
|---|---|---|
| `status` / `cleanup` | `local_cleanup_pending` → `external_erasure_pending` (app done, website pending) → `cleanup_completed` (website acknowledged) | **App + website cleanup only.** `cleanup.website = acknowledged` never implies provider erasure (`external.website.implies_provider_erasure = false`). |
