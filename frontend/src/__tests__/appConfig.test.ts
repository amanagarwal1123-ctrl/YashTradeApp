 ...
 */
const PREVIEW = 'https://yash-trade-backend.preview.emergentagent.com';
...
describe('app.config.js backend origin resolution', () => {
  ...
  it('release build never ships a preview-container origin: it resolves to the canonical production backend', () => {
    ...
    expect(loadConfig({ NODE_ENV: 'production', EXPO_PUBLIC_BACKEND_URL: 'https://yash-tryon-test.preview.emergentagent.com/' }).backendUrl).toBe(PRODUCTION);
