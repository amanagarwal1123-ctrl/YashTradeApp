...
export default function ProductCatalog() {
  ...
  useFocusEffect(useCallback(()=>{if(CONTENT_ROLES.includes(user?.role||''))load();return()=>{generation.current++;};},[user,load]));
  useEffect(()=>{const t=setTimeout(()=>{setPage(1);setTerm(search);},350);return()=>clearTimeout(t);},[search]);
  if(!CONTENT_ROLES.includes(user?.role||''))return <SafeAreaView style={ui.guard}><Text testID="catalog-access-denied" style={ui.error}>Admin or Upload Executive access required</Text></SafeAreaView>;
  return <SafeAreaView style={ui.screen}><KeyboardAvoiding style={ui.screen}>
