...
| **`REVIEW_ACCESS_ENABLED`** | yes for store-review accounts | **Declared** in preview `backend/.env` with the real value `true` (carried into production by the republish; keep it). It switches on the store-review environment: the `review__*` prefixed collections **inside `DB_NAME`** — no second database, no extra database right, no `REVIEW_DB_NAME` (that setting was removed on 13 Sep; delete it from Secrets if it still exists there, it is ignored). Off/placeholder → reviewer login 503 `REVIEW_UNAVAILABLE`, `flows.review.issues=["REVIEW_ACCESS_DISABLED"]`, nothing created. If the prefixed storage cannot be initialised the app **still starts**: `flows.review.usable=false`, `issues=["REVIEW_STORAGE_UNAVAILABLE"]`, reviewer sign-in 503; production flows are unaffected. |
...
Follow `STORE_REVIEW_ACCESS.md`. Primary path: the production app → Panel → **Store review** (owner administrator only, fresh OTP per action; keys shown once → COPY PRIVATE NOTE / DOWNLOAD .TXT). Equivalent CLI: `Provision-ReviewAccess.ps1 -Environment production -DbName <DB_NAME> …` — no `REVIEW_DB_NAME`, no redeploy: the `review__` collections live inside the production `DB_NAME` (`REVIEW_ACCESS_ENABLED=true` is already declared). Production reviewer keys are written only to a private file on your PC and verified against the deployed backend by the script itself. Exit code **0** = every key proved; **1** = blocked before any change and before any request (including `-VerifyNote` against a backend other than the one pinned in the note); **2** = keys issued/kept but verification failed or incomplete → fix the cause and re-run the same `-VerifyNote <note> -ApiBaseUrl <pinned backend>` command (read-only, no connection string). Reviewer keys open the **synthetic review copy only** — they never grant, prove or replace admin status on the production database.
...
| `POST /api/auth/review/login` with an unknown reviewer | 401 `REVIEW_CREDENTIALS_INVALID` when `REVIEW_ACCESS_ENABLED=true` and the prefixed review storage is usable; 503 `REVIEW_UNAVAILABLE` when it is off or the storage could not be initialised |
