...
| **`REVIEW_DB_NAME`** | yes for store-review accounts | **Declared** in preview `backend/.env` as `jewellers_app_review` (the working, isolated preview review database — distinct from preview `DB_NAME=jewellers_app`). Production: set a **different** name such as `<DB_NAME>_review`. Must differ from `DB_NAME`; when absent or a placeholder, reviewer login answers 503 and never touches production data. |
...
Follow `STORE_REVIEW_ACCESS.md` (`Provision-ReviewAccess.ps1`). Requires `REVIEW_DB_NAME` set in production Secrets and redeployed first; production reviewer keys are written only to a private file on your PC and verified against the deployed backend by the script itself. Exit code **0** = every key proved; **1** = blocked before any change; **2** = keys issued/kept but verification failed or incomplete → fix the cause and re-run with `-VerifyNote <note>` (read-only, no connection string). Reviewer keys open the **synthetic review copy only** — they never grant, prove or replace admin status on the production database.

## 5. Real OTP test — authorised for 9999813334 only

**Keys do not grant admin status.** Neither `STAFF_SERVICE_KEY` nor the reviewer keys change any role: restoring staff sign-in still requires §3 — the approved same-ID correction of 9999813334 (`bcdf18c9-dc87-4d46-b580-30cf519103df`), history preserved. "Admin login restored" may be claimed only after an authenticated **app** session and an authenticated **website** session both return `/auth/me` with that ID and `role=admin`.

Run only after sections 1 and 3 are complete (production redeployed, role repaired). One initial SMS per test, at most one resend after the normal 60-second cooldown, stop on any unexpected failure, no other recipients. (The single **preview** SMS on 12 September — see *Status* — was a transport test of the preview backend only and counts for nothing here.)
